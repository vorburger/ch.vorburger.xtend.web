...
class GradleWrapperWriter {
    ...
    def installGradleWrapper(File dir) {
        ...
        wrapperDir.mkdirs
        extractWrapperJAR(new File(wrapperDir, "gradle-wrapper.jar"))
        Files.write(gradleWrapperProperties, new File(wrapperDir, "gradle-wrapper.properties"), Charsets.UTF_8)
        val gradlewFile = new File(dir, "gradlew")
        Files.write(gradlew, gradlewFile, Charsets.UTF_8)
    ...
    def protected extractWrapperJAR(File file) {
        ...
        if (!urlString.startsWith("jar:file:/")) {
            throw new IllegalStateException("gradle-wrapper.jar on classpath, but not a jar:file:/ URI: " + urlString)
        }
        val wrapperJarFileName = urlString.substring(9, urlString.indexOf(".jar!/") + 4)
        val wrapperJarFile = new File(wrapperJarFileName)
